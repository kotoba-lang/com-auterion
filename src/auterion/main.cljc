(ns auterion.main
  "Kotodama WASM entrypoint for the Auterion clean-room actor (L4) — Clojure port.
  ...
  facts (`auterion.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "auterion")
(def tier "L4")
...
(def entity-specs
  [{:entity "Drone"      :plural "drones"      :id-prefix "auterion_dro"
   ...
   {:entity "Telemetry"  :plural "telemetries" :id-prefix "auterion_tel"
...
(defn emit-facts
  "EAVT facts for one record: {\"auterion.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "auterion-compat" :tier tier :entities entities} 200])
...
(defn start! [] :auterion-compat/ready)
